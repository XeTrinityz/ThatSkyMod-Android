...
void Unlock(const std::string& name, bool acknowledge) {
    queue::Enqueue("TUnlock(\"" + EscapeLuaString(name) + "\", " + (acknowledge ? "true" : "false") + ")");
}
...
void PlaySeasonFinale() {
    queue::Enqueue("TPlaySeasonFinale()");
}
...
void GenericShopNpcHint(const std::string& shopName, const std::string& iconName, const vec3& position) {
    std::ostringstream script;
    script << "TGenericShopNpcHint(\"" << EscapeLuaString(shopName) << "\", \""
           << EscapeLuaString(iconName.empty() ? "UiMenuShop" : iconName) << "\", "
           << position.x << ", " << position.y << ", " << position.z << ")";
    queue::Enqueue(script.str());
}
